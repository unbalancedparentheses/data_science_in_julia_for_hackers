...
begin
	...
	plot(imresize(im, Int(floor(scale*size(im)[1])), Int(floor(scale*size(im)[2]))))	
end
```
...
```{julia , echo=FALSE} 
plot(load("images/venn.png") |> (x -> imresize(x, Int(floor(0.7*size(x)[1])), Int(floor(0.7*size(x)[2])))))
```
...
begin
	...
	plot(imresize(conv_net, Int(floor(scale_two*size(conv_net)[1])), Int(floor(scale_two*size(conv_net)[2]))))
end
```
...
```{julia chap_10_process_images , CACHE = TRUE} 
begin
...
```{julia chap_10_readdir , CACHE = TRUE,results = FALSE} 
begin
	bee1_dir = readdir("10_bees_vs_wasps/preprocessed_data/bee1")
	bee2_dir = readdir("10_bees_vs_wasps/preprocessed_data/bee2")
	wasp1_dir = readdir("10_bees_vs_wasps/preprocessed_data/wasp1")
	wasp2_dir = readdir("10_bees_vs_wasps/preprocessed_data/wasp2");
end;
```
```{julia chap_10_load , CACHE = TRUE,results = FALSE}
begin
	# we load the pre-proccessed images
	bees1 = load.("10_bees_vs_wasps/preprocessed_data/bee1/" .* bee1_dir)
	bees2 = load.("10_bees_vs_wasps/preprocessed_data/bee2/" .* bee2_dir)
	wasp1 = load.("10_bees_vs_wasps/preprocessed_data/wasp1/" .* wasp1_dir)
	wasp2 = load.("10_bees_vs_wasps/preprocessed_data/wasp2/" .* wasp2_dir);
end;
```
...
```{julia chap_10_train , CACHE = TRUE } 
...
```{julia chap_10_train_two , CACHE = TRUE} 
