...
Now that we have made some historical overview of the topic, we can start designing our spam filter. Let's import the packages we will be using
along the chapter.
...
email set. We can see that words like 'win' –which in our training set appears in the form of 'won', but for the moment let's consider they are
the same– and 'product' appear rather commonly in our training data. So we will expect $P(email|spam)$ to be relatively high in this fake and simple example, 
as it contains words that are repeated among our spam emails data.

The technique we applied to consider –in principle– two different words as the same is called **lemmatization**. It is a standard technique in 
linguistics which groups together inflected forms of a word, letting us consider 'win' and 'won', in our example, as the same word.
...
A good practice when dealing with models that learn from data like the one we are going to implement, is to divide our data into two: a training 
...
data the model has never seen. This way we may be sure that the model is not tricking us. In Julia, the package *MLDataUtils* has some nice 
functionalities for data manipulations like this. We will use the functions *splitobs* to split our dataset in a train set and a test set and 
*shuffleobs* to randomize the order of our data in the split.
...
$P(word_i|spam) = \frac{N_{word_i|spam} + \alpha}{N_{spam} + \alpha N_{vocabulary}}$ (1)

and analogously, for ham emails,

$P(word_i|ham) = \frac{N_{word_i|ham} + \alpha}{N_{ham} + \alpha N_{vocabulary}}$ (2)
...
same words too), and finally, $N_{vocabulary}$ is the total number of unique words in the dataset. 
You might be surprised to read the `$α$` value in the equations. This is what is called a  *smoothing parameter*. To understand why is it needed,
let's think for a moment what would happen when we receive an email having a word that is not present neither in the spam words training set nor
in the ham words trainig set. As we have seen, to calculate the probability of the email being a spam email, we should use 

$P(email|spam) = \prod_{i=1}^{n}P(word_i|spam) = P(word_0|spam)P(word_1|spam)...P(word_np|spam)$

where $P(word_np|spam)$ stands for the probability of the word that is not present in our dataset. What probability should be assigned to this
word? One way to handle this could be to simply ignore that term in the multiplication. In other words, assigning $P(word_np|spam) = 1$.
Without thinking about it too much, we can conclude that this doesn't make any sense, since that would mean that the probability to find that word
in a spam (or ham, too) email would be equal to 1.
A more logically consistent approach would be to assign 0 probability to that word. But there is a problem: with $P(word_np|spam) = 0$ we can 
quickly see that

$P(word_0|spam)P(word_1|spam)...P(word_np|spam) = 0$

This is the motivation for introducing the smoothing parameter $\alpha$ into our equation. In a real-word scenario, we should expect that words not
present in our training set will appear, and altough it makes sense that they don't have a high probability, it can't be 0. When such a word
appears, the probability assigned for it will be simply

$P(word_np|spam) = \frac{N_{word_np|spam} + \alpha}{N_{spam} + \alpha N_{vocabulary}} = \frac{0 + \alpha}{N_{spam} + \alpha N_{vocabulary}} = \frac{\alpha}{N_{spam} + \alpha N_{vocabulary}}$

In summary, $α$ is just a smoothing parameter, so that the probability of finding a word that is not in our dataset, doesn't go down to 0.
Since we want to keep the probability for these words low enough, it makes sense to use $\alpha = 1$. 

All the information about words and their probabilities will be particular for our dataset, so a clever way to aggregate all this is to use a 
Julia *struct*. We can define the attributes of the struct that we will be using over and over for the prediction. Below we can see the 
implementation. The relevant attributes  of the struct will be *words_count_ham* and *words_count_spam*, two dictionaries containing the frequency
of appearance of each word in the ham and spam datasets, *N_ham* and *N_spam* the total number of words appearing in each category, and finally 
*vocabulary*, an array with all the unique words in our dataset.
...
We are now almost ready to make some predictions and test our model. The function below is just the implementation of the formulas (1) and (2) that 
we have already talked about. It will be used internally by the next function defined, `spam_predict`, which will receive a new email –the one we 
