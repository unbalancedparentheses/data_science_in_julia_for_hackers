...
```{julia,results = FALSE} 
θ = 45
chain_uniform = sample(gravity_uniform(Δt, Δx, θ), HMC(ϵ, τ), iterations, progress=false);
```
...
```{julia,results = FALSE} 
chain_normal = sample(gravity_normal(Δt, Δx, θ), HMC(ϵ, τ), iterations, progress=false)
```
...
```{julia,results = FALSE} 
chain_lognormal = sample(gravity_lognormal(Δt, Δx, θ), HMC(ϵ, τ), iterations, progress=false)
```
...
```{julia, results = FALSE} 
chain_uniform_angle = sample(gravity_angle_uniform(Δt, Δx, θ), HMC(ϵ, τ), iterations, progress=false)
```
