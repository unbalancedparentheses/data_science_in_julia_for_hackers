...
Suppose you landed on Mars by mistake and you want to leave that rocky planet and return home. To escape from a planet you need a very important piece of information: the escape velocity. 

What is the escape velocity on Mars?


We are going to use the same experiment that Newton thought of when thinking about escaping from the gravity of Earth. Gravity pulls us down, so if we shoot a cannonball, as in the sketch shown below, what will happen? For some velocities, the cannonball will return to Earth, but there's a velocity at which it escapes since the gravitational pull is not enough to bring it back to the surface. That velocity is called **escape velocity**."
...
where *r* is the radius of the planet and *g* the constant of gravity at the surface. Suppose that we remember from school that the velocity of escape from Earth is $11\frac{km}{s}$ and that the radius of Mars is half of the Earth's. "
...
md"We are going to calculate the constant $g_{mars}$ just throwing stones. We are going to explain a little of the equations regarding the experiment. The topic we need to revisit is *Projectile Motion*. "

# ╔═╡ 424b96de-1884-11eb-06fe-032b13ea20eb
md" ## Projectile Motion"

# ╔═╡ 4aa3534c-1884-11eb-0c30-e387db85affe
md" Gravity pulls us down to Earth, or in our case, to Mars. This means that we have an acceleration, since there is a force. Recalling the Newton equation:

$\overrightarrow{F} = m * \overrightarrow{a}$
 where *m* is the mass of the object, $\overrightarrow{F}$ is the force (it's what makes us fall) and $\overrightarrow{a}$ is the acceleration, in our case is what we call gravity $\overrightarrow{g}$. The arrow $\overrightarrow{}$ over the letter means that the quantity has a direction in space, in our case, gravity is pointing to the center of the Earth, or Mars.

*How can we derive the motion of the stones with that equation?*

In the figure below we show a sketch of the problem: We have the 2 axes, *x* and *y*, the *x* normally is parallel to the ground and the *y* axis is perpendicular, pointing to the sky. We also draw the initial velocity $v_0$ of the projectile, and the angle $\theta$ with respect to the ground. Also it's important to notice that the gravity points in the opposite direction of the *y* axis."

# ╔═╡ 677b95c2-1888-11eb-18be-0de8ca145819
md" *But what is the trajectory of the projectile? And how do the coordinates x and y evolve with time?*

If we remember from school, the equations of *x* and *y* over time are: 
"
...
We can imagine what happens if we throw a stone to the sky: the stone starts to go up and then, at some point, it reaches the highest position it can go. Then, the stone starts to go down. 

*How does the velocity evolve in this trajectory?*

Since the beginning, the velocity starts decreasing until it has the value of 0 at the highest point, where the stone stops for a moment, then it changes its direction and start to increase again, pointing towards the ground. Plotting the evolution of the height of the stone, we obtain the plot shown below.  We see that, at the beginning the stone starts to go up fast and then it slows down. We see that for each value of *y* there are 2 values of *t* that satisfy the equation, that's because the stone passes through each point twice, except for the highest value of *y*."
...
md"So, in the example we have just explained, we have that the throwing angle is θ=90°, so sin(90°)=1, the trajectory in *y* becomes:"
...
md" It's worth noting that at each value *y* of the trajectory, the velocity could have 2 values, just differing in their sign, meaning it can have 2 directions, but with the same magnitude. *So keep in mind that when you throw an object to the sky, when it returns to you, the velocity will be the same as the one you threw it with.*"
...
*how do we set up the experiment and what do we need to measure?*

The experiment set up will go like this:
- One person will be throwing stones at an angle.
- The other person will be watching from some distance far away, measuring the time Δt taken from when the stone is thrown until it hits the ground. The other measurement we will need is the distance Δx the stone travelled.
- Also, for the first iteration of the experiment, suppose we only keep the measurements with an initial angle θ~45° (we will loosen this constraint in a bit). 
"
...
md"*Now, how do we estimate the constant g from those points?*

Using the equations of the trajectory, when the stone hits the ground, y(t) = 0, since we take the start of the *y* coordinate at ground level (neglecting the initial height with respect to the maximum height), so finding the other (initial) point that fulfills this equation, we find that:
...
where *m* is the slope of the curve and *b* is called the intercept. In our case, if we take *x* to be $\frac{t_{f}^2}{2}$, the slope of the curve is *g* and the intercept is 0. So, in our linear model we are going to propose that each point in the curve is:

$\mu = m*x + b$

$y \sim Normal(\mu,\sigma^2)$

So, what this says is that each point of the regression is drawn from a gaussian distribution with its center corresponding with a point in the line, as shown in the plot below."
...
md"Plotting the posterior distribution for p we see that the values are mostly between 2 and 5, with the maximum near 3.8. Can we narrow the values we obtain?"
...
md"As a second option, we can propose a *Gaussian distribution* instead of a uniform distribution for *g*, like the one shown below, with a mean of 6 and a variance of 2, and let the model update its beliefs with the points we have."

# ╔═╡ bb570a5e-1a19-11eb-26ca-0dd1a523dc5d
begin
plot(Normal(6,2), legend=false, fill=(0, .5,:lightblue))
...
If we observe the prior distribution proposed of *g*, we see that some values are negative, which makes no sense because if that would be the case when you throw the stone, it would go up and up, escaping from the planet. 

We propose then a new model for not allowing the negative values to happen. The distribution we are interested in is a LogNormal distribution. In the plot below is the prior distribution for g, a LogNormal distribution with mean 1 and variance of 0.5."
...
md"Now that we have a good understanding of the equations and the overall problem, we are going to add some difficulties and we will loosen a constraint we have imposed: suppose that the device employed to measure the angle has an error of 15°, no matter the angle. 

*We want to know what are the most convenient angles to do the experiment and to measure or if it doesn't matter.*"

# ╔═╡ 730d6e62-1a25-11eb-09cd-4133b7f6c59a
md"To do the analysis we need to see how the angle influences the computation of *g*, so solving the equation for *g* we have:

$g = \frac{2*tg(\theta)*\Delta x}{t^{2}_f}$
"

# ╔═╡ c32ae204-1ab5-11eb-0600-ab56ad81dad2
md"We can plot then the tangent of θ, with an error of 15° and see what is its maximum and minimum value:"
...
md"But we don't care about the absolute value of the error, we want the relative error, so plotting the percent error we have:"

# ╔═╡ efcdbdd8-1ab8-11eb-0dc8-f531dc2623af
begin
er= tan.(deg2rad.(angles .+ error)) .- tan.(deg2rad.(angles .- error))
perc_error = er .* 100 ./ μ
plot(angles, er .* 100 ./ μ, xlim=(5,70), ylim=(0,200), color="lightblue", legend=true, lw=3, label="Percent error")
...
md"So, now we see that the lowest percent error is obtained when we work in angles near 45°, so we are good to go and we can use the data we measured adding the error in the angle."

# ╔═╡ fe1996b6-1ac0-11eb-0f46-230fdebf0395
md"We now define the new model, where we include an **uncertainty in the angle**. We propose a *Uniform prior* for the angle centered at 45°, the angle at which we think the measurement was done."
...
begin
	...
	xlabel!("g_mars")
	ylabel!("Probability")
	title!("Posterior distribution for g, including uncertainty in the angle",titlefont=(12))
end
...
$g_{Earth}\approxeq 9.8\frac{m}{s^2}$
...
md"Finally, we obtained the escape velocity of Mars."
